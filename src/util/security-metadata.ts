...
const AUTHENTICATION_CLASS_KEY = MetadataAccessor.create<unknown, ClassDecorator>(
  'authentication:class',
);
const AUTHENTICATION_METHOD_KEY = MetadataAccessor.create<unknown, MethodDecorator>(
  'authentication:method',
);
const AUTHORIZATION_CLASS_KEY = MetadataAccessor.create<unknown, ClassDecorator>(
  'authorization:class',
);
const AUTHORIZATION_METHOD_KEY = MetadataAccessor.create<unknown, MethodDecorator>(
  'authorization:method',
);
const LEGACY_AUTHENTICATION_CLASS_KEY = MetadataAccessor.create<unknown, ClassDecorator>(
  'authentication:metadata',
);
const LEGACY_AUTHENTICATION_METHOD_KEY = MetadataAccessor.create<unknown, MethodDecorator>(
  'authentication:metadata',
);
const LEGACY_AUTHORIZATION_CLASS_KEY = MetadataAccessor.create<unknown, ClassDecorator>(
  'authorization:metadata',
);
const LEGACY_AUTHORIZATION_METHOD_KEY = MetadataAccessor.create<unknown, MethodDecorator>(
  'authorization:metadata',
);
...
export function collectControllerSecurityMetadata(
  ctor: Function,
): ControllerSecurityMetadata | undefined {
...
function readClassMetadata(target: Function, keys?: ClassMetadataKey[]): unknown {
  ...
  for (const key of keys) {
    const value =
      MetadataInspector.getClassMetadata(key, target) ??
...
function readMethodMetadata(
  target: object,
  methodName: string,
  keys?: MethodMetadataKey[],
): unknown {
  if (!keys?.length) return undefined;
  for (const key of keys) {
    const value =
      MetadataInspector.getMethodMetadata(key, target, methodName) ??
...
export function mergeMethodAliasMaps(
  ...maps: (MethodAliasMap | undefined)[]
